...
def crawl_webPage(n):
    # web_url='http://www.zuowen.com/chuzhong/chuyi/yilunwen/index'
    web_url='http://www.zuowen.com/chuzhong/chusan/huati/index'
    # doc=pq(web_url)
    page_url=[]
    title_list=[]
    # content_list=[]
    for i in range(2,n+1):
        ...
        r.encoding = 'gb2312'
        doc = pq(r.content)
        for i in doc('.artlist_l .artbox_l .artbox_l_t a').items():
...
def crawl_content(page, title):
    ...
    for tmp in page_url:
        ...
        doc=pq(r.content)
        content_tmp=''
        for i in doc('.con_content p').items():
            content_tmp+='\n' + i.text()
    ...
    df_all = pd.DataFrame(data)
    df_all.to_csv('./huati3.csv',index=False,encoding='gb18030')

def main():
    page, title = crawl_webPage(150)
    print(len(page))
